...
// Constructor
Pipeline::Pipeline(VulkanDevice& device, SwapChain& swapChain) : device(device), swapChain(swapChain) {
}
...
bool Pipeline::createGraphicsPipeline() {
  try {
    ...
    // Note: viewport and scissor are dynamic states, so we only need counts
